...
def calc_qm_vs_md_frequencies(mypath, compound, qm , md_hessian):
    ...
    mean_percent_error = write_vibrational_frequencies(qm_freq, qm_vec, md_freq, md_vec, qm, mypath, compound)
    plot_frequencies(mypath, compound, qm_freq, md_freq, mean_percent_error)


def plot_frequencies(mypath, compound, qm_freq, md_freq, mean_percent_error):
    ...
    plt.plot(n_freqs, md_freq, linewidth=3, label='MD')
    plt.tight_layout()
    plt.legend(ncol=2, bbox_to_anchor=(1.03, 1.12), frameon=False)
    f.savefig(f"{mypath}{compound}/FREQUENCY/frequencies.pdf", bbox_inches='tight')
    plt.close()
...
def write_vibrational_frequencies(qm_freq, qm_vec, md_freq, md_vec, qm, mypath, compound):
    """
    Scope:
    ------
    ...
    JOBNAME_qforce.nmd : MD eigenvectors that can be played in VMD with:
                                vmd -e filename
    """
    freq_file = f"{mypath}{compound}/FREQUENCY/frequencies.txt"
    nmd_file = f"{mypath}{compound}/FREQUENCY/frequencies.nmd"
    errors = []
    ...
    with open(nmd_file, "w") as nmd:
        nmd.write(f"nmwiz_load {compound}_qforce.nmd\n")
        nmd.write(f"title {compound}\n")
        nmd.write("names")
